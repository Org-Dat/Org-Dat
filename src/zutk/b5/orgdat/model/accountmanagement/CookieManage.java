...
public class CookieManage{ 
	...
	public String createCookie(long user_id,String ip_address,String user_agent){
	    ...
	    try{
	        ...
	        dc.stmt.executeUpdate();
			Date now = new Date();
			dc.stmt = (dc.conn).prepareStatement("insert into history (user_id,ip_address,user_agent,signin_time,signout_time) values(?,?,?,?,?)");
			dc.stmt.setLong(1, user_id);
	        dc.stmt.setString(2, ip_address);
	        dc.stmt.setString(3, user_agent);
	...
	public boolean deleteCookie(String iambdt,long user_id , String ip_address ,String user_agent){
	    ...
	    try{
			...
			dc.stmt.executeUpdate();
		    Date now = new Date();
			dc.stmt = (dc.conn).prepareStatement("update  history set signout_time=?  where user_id=? and ip_address = ? and user_agent = ? and signout_time='login' ");
